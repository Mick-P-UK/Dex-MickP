...
- Last updated: 2026-08-04 (added md-to-docx v1.0: markdown to formatted Word document, optional PDF via Word COM. Promoted from the one-off converter written for the HOC Hochschild Mining research report the same day - Mick's call to keep it, since every Ron report and SOP #6 quarterly needs a DOCX. Real Word structure (Heading styles, native tables, embedded images, lists, hyperlinks, code blocks, page breaks), mandatory provenance footer with --no-footer for press copies, and an ASCII pass that reports rather than mangles - pound and cent signs deliberately exempt, matching the non-ascii-sweep REVIEW_IGNORE decision of 2026-08-02. Both skill files verified pure ASCII and byte-identical vault vs user-level. SOP index entry #10 added. THEN bumped to v1.1 the same evening on Mick's go-ahead: added docx_to_pdf.py, a standalone Word-to-PDF entry point with batch mode, after a vault-wide search found NO docx-to-pdf implementation anywhere - despite the Freedom Blueprint newsletter SOP Phase 5 calling for one since July, so it had been improvised every month. Deliberately a second ENTRY POINT in the same skill rather than a separate skill, to avoid a near-duplicate name next to md-to-docx (the discovery failure mode that hit the CREI run on 2026-07-22) and to keep one Word COM implementation. Upgraded to ExportAsFixedFormat for heading bookmarks + tagged PDF. Newsletter SOP Phase 5 now points at the script.)
...
| md-to-docx | "make a Word document from this MD" / "convert this markdown to Word" / "turn the report into a docx" / "create a Word version" / "convert this Word doc to PDF" / "PDF this document" / "make a PDF of the newsletter" | mick-cedric | V (+ user-level C:\Users\pavey\.claude\skills; M pending) | active | v1.1 (2026-08-04): TWO entry points in one skill. (1) md_to_docx.py - markdown to formatted Word document, optional PDF. Real Word structure - Heading styles, native tables with shaded header rows, embedded images (resolved relative to the source md), two-level bullets, numbered lists, blockquotes, fenced code blocks, clickable hyperlinks, horizontal rules, forced page breaks, and the Ron/Nina "**Header:**" title convention. Mandatory left-aligned provenance footer (--no-footer for going-to-press). ASCII pass auto-converts typographic punctuation, leaves the pound/cent signs alone per the non-ascii-sweep REVIEW_IGNORE decision, and REPORTS anything else with line numbers instead of mangling it. Frontmatter dropped by default. (2) docx_to_pdf.py - standalone Word to PDF for any .docx that did NOT come from markdown (newsletter, year-end pack, webinar user guides): single file, whole folder (one Word launch per batch), or a mix; --recurse, --skip-existing, --no-bookmarks, --screen; sources opened READ-ONLY; ~$ lock files ignored. Uses ExportAsFixedFormat so the PDF carries BOOKMARKS from the Word headings plus tagging and doc properties (verified 12 headings to 12 nested bookmarks on HOC), with automatic SaveAs fallback. ALSO the shared PDF engine - md_to_docx.py --pdf imports convert_one() from it, so one implementation, no drift. Both scripts pure ASCII (python-docx for the DOCX half; pywin32 + Word for the PDF half). Does NOT build the newsletter, but IS now the newsletter SOP Phase 5 PDF step - that step had NO script behind it since July and was being improvised monthly. Born out of the HOC research report conversion, its first live use. |
