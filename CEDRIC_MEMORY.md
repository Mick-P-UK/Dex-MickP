# CEDRIC MEMORY
**Last Updated:** 2026.08.06 (Thu, Claude Code, evening 18:06) - AX-TREE FILTER MAPPING FROM A VIDEO BRIEF (Session 21). Mapped the ShareScope Filter area (sub-toolbar, Apply/Manage modal, Edit-details dialog, fork-a-filter workflow) into the ax-trees master map as Section 2B (v1.7), driven entirely by a narrated Snagit .mp4 Mick recorded. Proved a REUSABLE video-brief pipeline (imageio-ffmpeg frame extraction + faster-whisper transcription + frame reading) - Mick will brief future tasks by video. New standing rule: browser HEADLESS by default (headed only when a brief says to watch). Fork behaviour confirmed by a controlled create/verify/delete cycle (no residue). Also found: ShareScope Delete removes a saved filter immediately, no confirmation.
**Last Updated:** 2026.08.06 (Thu, Claude Code, early afternoon 12:47) - AX TREES AUTOMATION RELOCATED + OWN GITHUB REPO. Moved the live ax-trees-automation project out of the now-cloud-only Cowork folder to C:\Vaults\ax-trees-automation (its own git repo, 1063 files, history intact; PowerShell Move-Item - plain mv was blocked by the scheduled autocommit task + shell cwd). Repointed the "ax-trees-automation git autocommit" scheduled task and corrected the old Cowork path in 11 operational scripts (commit a09f45f in that repo). Archived the stale 1-May snapshot Dex-MickP\ax-trees-automation (22 files) to 07-Archives/Projects/ax-trees-automation-snapshot-2026-05-01-SUPERSEDED via git mv. Confirmed skills/ax-mapper is a LIVE deployed skill (registered + run 11 Jul), NOT stale - kept, and its two dead Cowork path refs fixed. Committed the Dex changes surgically (58a78e5) and pushed to Mick-P-UK/Dex-MickP; created a PRIVATE repo github.com/Mick-P-UK/ax-trees-automation (gh, HTTPS auth) and pushed main after a clean secret scan. NEXT: Mick starts the ShareScope AX-tree mapping in a fresh session rooted in C:\Vaults\ax-trees-automation (mapper BUILT, not yet live-tested). Full detail: _handovers/LATEST.md.
**Last Updated:** 2026.08.05 (Wed, Claude Code, late morning 11:21) - HECLA (HL) SOP#1 US-STOCK RUN (SEC-primary) + TWO SHARESCOPE FIXES + DOCX/PDF. Ran the full research pipeline on Hecla Mining (NYSE:HL), a US stock, hybrid "Both, SEC as primary" (Mick's choice): pulled SEC EDGAR filings (CIK 719413 - FY2025 10-K, Q2 2026 10-Q, and the Q2/H1 2026 8-K earnings release released the day before), stripped HTML to text, archived to the HL base-data folder, built NEW NotebookLM notebook 781ebf35 (SEC filings + ShareScope financials as text; 9 sources ready). Ron report = BUY (net cash ~$483m, debt-free, H1 FCF $279m, ~18.8x fwd P/E; key level $13 - the 18-year breakout shelf now support); Ava audit 25/25 clean (units cents-vs-dollars and continuing-vs-discontinued traps both navigated). Saved to Research-Log/Research/HL/2026.08.05...v2.md + Company Profile + index (23 companies); branded FINAL DOCX+PDF in the 5 Aug IC webinar folder (Documents\0.1 - Projects (n)\2026.08.05 - IC.Webnr\HL - Hecla Mining - Aug'26). Mick presents tonight; NOT in Notion (present-first hold, as HOC). Verified HL = Hecla not Hargreaves Lansdown (Historical) via the search dropdown. TWO PRODUCTION FIXES verified live: (1) sharescope_chart.py sets DAILY candles via the Bars dropdown '1 day' - the period WINDOW and bar INTERVAL are separate controls, and the bottom ss-btn-setting presets retain the last period so are unreliable; (2) sharescope_search.py company-picker logs all DROPDOWN candidates, WARNS on ambiguous tickers, and takes expected_exchange= to force the right instrument and abort rather than guess. FINDING: ShareScope CSVs fail NotebookLM as FILE uploads (Unknown/error) regardless of width - trimming to 7 years did NOT help; add as TEXT (auto-memory saved). PENDING: Mick to decide whether to codify the SEC-primary US-stock branch in SOP#1. Full detail: _handovers/LATEST.md.
...
## Recent session: 2026.08.06 (Thursday, Claude Code, evening) - AX-tree Filter mapping from a video brief (Session 21 on ax-trees-automation)

Mick opened a new AX-tree mapping task and asked whether Cedric could work from a recorded video. He can: Cedric cannot play video/audio directly, so the pipeline is imageio-ffmpeg (self-contained ffmpeg, no system install) to slice frames + extract audio, faster-whisper (small.en) to transcribe the narration into timestamped segments, then read the frames as images and align them to the narration. A 4m16s narrated Snagit .mp4 drove the whole task. This is now Mick's preferred brief format ("I'll record a video") - see auto-memory video-brief-workflow.md.

Task: map the ShareScope "Filter" area. Did it map-first, read-only, per the project's AX-MAPPING-PLAYBOOK, using the existing sharescope-login.js. Harvested (3 refining passes) the list sub-toolbar, the Filter dropdown (ul#tour-filter-menu, li[data-cmd=...]), the "Apply a filter to the list" modal (#dialog1: list #add-filter-dlg-list, buttons #add-filter-dlg-{new|delete|copy|rename|lock|library}-btn, apply #add-filter-dlg-ok-btn), and the "Edit filter details" dialog (#dialog2: #filter-name-dlg-name/-desc/-list/-filter-setting/-ok-btn). All selectors confirmed live, none guessed. Committed as master map Section 2B (v1.7).

Fork-a-filter workflow confirmed by ONE controlled cycle (fork-cycle-test.js): copied the Starting Point template, renamed the copy to AXMAP_TEST_DELETE_ME via Edit details, verified it appeared, deleted it, verified gone - no residue. Delete has NO confirmation prompt.

New standing rule (Mick, mid-session): browser HEADLESS by default; headed only when a brief explicitly says to watch (a popping browser interrupts his video editing). Codified in the ax-trees CLAUDE.md and auto-memory headless-browser-default.md.

Files: ax-trees-automation repo - sharescope/sharescope-ax-tree-master.md (v1.7, Section 2B), CLAUDE.md (Browser Mode rule), mini-projects/sharescope-stock-filter/ (PROGRESS.md + scripts/harvest-filter.js + fork-cycle-test.js + DRAFT-map-section-2B.md), MINI-PROJECTS-MASTER.md, PIPELINE-PROGRESS.md, session-logs/ (session21 + pickup). Next: design the filter parameter schema and build sharescope-stock-filter.js on the Section 2B selectors. Resume with SESSION21-PICKUP.md.

---

## Recent session: 2026.08.06 (Thursday, Claude Code, early afternoon) - AX Trees automation relocated out of Cowork + own GitHub repo

Mick could not recall where the AX Trees automation lived; found it in three places. Because Cowork is now cloud-only and this project is local-execution (reads C:\Users\pavey\.env, drives a local browser), moved the live copy from C:\Vaults\Cowork\ax-trees-automation to C:\Vaults\ax-trees-automation (own git repo, history intact). Repointed the git-autocommit scheduled task and fixed 11 hardcoded Cowork paths (repo commit a09f45f). Archived the stale 1-May Dex snapshot to 07-Archives/Projects/ (git mv); confirmed skills/ax-mapper is a live deployed skill and kept it, fixing its two dead path refs. Dex changes committed surgically (58a78e5) and pushed; new PRIVATE repo github.com/Mick-P-UK/ax-trees-automation created and pushed. Next: start the ShareScope mapping in a fresh session rooted in the new folder.

---

## Recent session: 2026.08.05 (Wednesday, Claude Code, late morning) - Hecla (HL) US-stock SOP#1 run, SEC-primary, plus two ShareScope automation fixes

Full SOP#1 research run on **Hecla Mining (NYSE:HL)** - the first US-stock run since the pipeline matured. Mick chose a hybrid **"Both, SEC as primary"** approach.

- **Data-in (SEC-primary):** pulled SEC EDGAR filings (CIK 719413) with curl + a User-Agent header - FY2025 10-K, Q2 2026 10-Q, and the Q2/H1 2026 8-K EX-99.1 earnings release (released 4 Aug, the day before). Stripped HTML to text (BeautifulSoup), archived to the HL base-data SEC-Filings folder, and built a NEW NotebookLM notebook `781ebf35-92ca-42d7-b6af-033b44494f0c` ("HL - Hecla Mining_Updated:2026.08.05"). Notebook now has 9 ready sources: 3 SEC filings + 6 ShareScope financials as text.
- **Ticker safety:** "HL" also matches Hargreaves Lansdown (LSE, now Historical). Verified via the ShareScope search dropdown that NYSE:HL Hecla is the live first pick.
- **Report:** Ron = **BUY** (silver-leveraged on the pullback; net cash ~$483m, debt-free, H1 FCF $279m, ~18.8x fwd P/E; key level **$13**, the 18-year breakout shelf now support). Ava audit **25/25 clean** - the units (US cents vs dollars) and continuing-vs-discontinued-ops (Casa Berardi disposal) traps both handled. Saved to `06-Resources/Research-Log/Research/HL/2026.08.05 - HL - Hecla Mining - AI - Financial Analysis_v2.md`; Company Profile created; index now 23 companies.
- **Deliverables:** branded **FINAL** DOCX + PDF (8 pages, both charts, both tables) in `Documents\0.1 - Projects (n)\2026.08.05 - IC.Webnr\HL - Hecla Mining - Aug'26`. Mick presents at the 5 Aug IC webinar tonight; deliberately NOT logged to Notion yet (present-first hold, same as HOC).
- **Two production fixes (verified live):** (1) `sharescope_chart.py` now sets DAILY candles via the toolbar **`Bars` dropdown -> `1 day`** (`set_chart_periodicity()`); the period WINDOW and bar INTERVAL are separate controls, and the bottom `ss-btn-setting` presets retain the last-selected period so are unreliable. (2) `sharescope_search.py` company-picker now logs all **DROPDOWN VERIFICATION** candidates, WARNS on an ambiguous ticker, and accepts `expected_exchange=` to force the right instrument and ABORT rather than guess.
- **Finding:** ShareScope CSVs fail NotebookLM as **file** uploads (parsed "Unknown"/error) regardless of width - a 7-year trim did NOT help. Add them as **text** instead (the automated researcher already does; only manual `--type file` trips over it). Auto-memory `sharescope-csv-notebooklm-text-not-file` saved.
- **Pending:** Mick to decide whether to codify the SEC-primary US-stock path as a documented branch of SOP#1. See auto-memory `hl-us-stock-sop1-sec-primary-and-sharescope-fixes`.
...
Ran the full ShareScope + NotebookLM + Ron pipeline (SOP #1) on Hochschild Mining (HOC, LSE gold/silver producer). NotebookLM auth had expired; healed hands-free via the Playwright cookie re-export (headed browser auto-signed-in from stored creds - no input from Mick). Captured 6 ShareScope CSVs + a 12-month chart. Nina built notebook 6a2ecdfd-dcf8-46a8-a15f-9d72bf60cf9a (6 CSVs + 10 news sources, no dupes). At Mick's request, checked LSE RNS since the 11 Mar 2026 prelims - key item the 22 Jul 2026 H1 production report: H1 151,830 GEO (about half of reaffirmed 300-328k FY guidance), swung to NET CASH $51m (from net debt $22.7m at FY25), AISC running 5-10pc above the $2,157-2,320/GEO guide, realised gold $4,167/oz. Ron wrote the report: BUY, anchored to the 430-460p base, stop below 400p, confirmation pivot 500p. FY2025 actuals: turnover $1,182m (+24.7pc), gross margin 42.7pc, reported PBT $372.8m.

NEW PERSONA - AVA THE AUDITOR. Mick asked for an independent calculation checker as a permanent persona with an editable rulebook. Built Ava as a named sub-agent (chose "Ava", NOT "Annie the Auditor", to avoid clashing with the existing calendar Annie). On the HOC pilot she independently recomputed all 26 of Ron's calcs and caught a MATERIAL error: price-to-forward-NAV stated 1.87x was a pence-vs-cents mix-up (NAV per share is US cents, not pence); correct ~2.5x, matching ShareScope's own 2.4x. Plus a minor rounding fix (OCF ~760 -> ~766) and a missing calc-ID (effective tax rate). Corrected before issue.

Ava design (built to be easy to evolve): persona .claude/agents/ava.md (Dex-MickP + Writing-System, synced like Ron; subagent_type "ava" available after a Claude Code restart, interim via general-purpose + inline prompt). Editable rules pack lives OUTSIDE the persona at 06-Resources/Audit-System/ = rules.md (global rules R1-R10, Mick edits anytime) + examples.md (curated worked examples, HOC = example 1) + checklists/financial-analysis.md (report-type traps: EPS + NAV pence-vs-cents, reporting currency, net-debt basis, attributable-vs-consolidated). Ava re-reads the pack every run, so new checks need no code change. Made MANDATORY as Step 4.5 of the canonical SOP #1 (v2.1, the Cowork file); Ron's Step 4 now requires a Calculations Appendix. SOP index + memory project_ron_output_checker updated. Master agent registry created at 06-Resources/AGENTS.md (Cedric, Nina, Ron, Ava, calendar Annie, Dex Assistant, writing agents).

Deliverables (in Documents\0.1 - Projects (n)\2026.08.05 - IC.Webnr\HOC - 2026.08.04 - Micks Radar\): the audited report (Appendix A calcs + Appendix B audit), the 12-month chart PNG, a members-facing session transcript ("Cedric and the team"), and a Meet Cedric episode ("The Team That Audits Itself"). Original report also saved to the Research Log (Research/HOC/); company profile + _index updated (HOC now tracked, 22 companies).

HELD / PARKED (Mick, prepping the 5 Aug Inner Circle webinar): HOC NOT logged to the Notion Research DB yet (wants to present it at the webinar first); the Meet Cedric episode NOT logged to Content Studio yet (same reason); technical-analysis + production-analysis audit checklists parked; the stale vault mirror skills/sharescope-nlm-research/SKILL.md (old Nina-only v1) still needs reconciling up to canonical v2.1. Minor: HOC company-profile auto-tagged exchange "LSE:AIM" but HOC is LSE Main Market/FTSE 250 (index set to "LSE"; profile not corrected).

## Recent session: 2026.08.01 (Saturday, Claude Code, evening) - End-of-Month portfolio posting completed (3 drafts + index-comma convention)

- CONTEXT: resumed from LATEST handover (EOM routine). UK Active 10 Y1 draft 15585 was reviewed and PUBLISHED by Mick this session; task was to create the other three posts.
- Built the three via 04-Projects/2026.04.04-ShareScope-Automation/eom_post_drafts.py, adding post_us_active_10 (MID_YR1), post_uk_active_10_yr2 and post_us_active_10_yr2 (MID_YR2). Each was matched to its June predecessor pulled live from the WP REST API (tags 512/890/891, posts 15471/15468/15474) for exact house wording. Figures read off the captured images per SOP.
- RESULT: US Y1 = draft 15602 (down 6.89pc, $9,310.19, behind SP500 by 16.30pc, 1 txn ACMR exit). UK Yr2 = draft 15597 (up 61.82pc, GBP 16,182.41, +31.19pc vs ASX, 2 txns). US Yr2 = draft 15599 (up 79.56pc, $17,956.11, +52.22pc vs SP500, NO transactions -> text line, no image, per prior decision). Category + portfolio tag set on each; all drafts, Mick publishes.
- US Y1 DISCREPANCY (important pattern): the annotation box on the holdings image said 'Down by $698.63 [-6.98%]' (implies 9301.37) but the ShareScope Total row = Holdings 6463.36 + Cash 2846.83 = 9310.19. Mick chose the ShareScope truth 9310.19. Fix: re-ran format_holdings.py on the preserved eom/full/_raw panel with --total 9310.19 -> box now 'Down by $689.81 [-6.89%]'; old image backed up to eom/full/_superseded. The _raw panels are kept precisely so a holdings image can be re-formatted without a re-capture.
- NEW CONVENTION (Mick): benchmark INDEX values are written with NO thousands separator (5350.38, 5836.47, 1368.67 points); currency/portfolio values DO keep the comma. De-comma'd index values in all four live posts via targeted REST content patches (no image re-upload, statuses preserved incl. published 15585), and changed eom_post_drafts.py to format idx_start/idx_end/idx_points with :.2f. Saved auto-memory index-values-no-thousands-comma.md.
- Also hardened eom_post_drafts.py: percentages print :.2f (US Y1 'behind' had shown 16.3 not 16.30); 'down by' wording uses abs() so it reads 'down by 6.89pc' not 'down by -6.89pc'.
- Note: the 04-Projects/2026.04.04-ShareScope-Automation folder is git-IGNORED, so script changes are not backed up by the vault git; only vault-tracked files (memory, changelog, handover) commit.
- OPEN / NEXT: (1) Meet Cedric episode for the EOM build still not logged; (2) benchmark-fetcher stale Yahoo ^FTAS fix (used ShareScope ASX 5836.47 manually; Indices Monthly Performance spreadsheet NOT updated this run); (3) clean up orphaned early UK media 15583/15584/15586/15587; (4) wrap capture + posting into one overarching routine/SOP.
