...
if token:
    print(f"   [x] API token available: {token[:50]}...")
else:
...
if cache:
    print(f"   [x] Cache available with {len(cache.get('documents', {}))} documents")
else:
...
try:
    ...
    print(f"   [x] Retrieved {len(meetings)} meetings")
    if meetings:
        first = meetings[0]
...
try:
    ...
    print(f"   [x] Found {len(results)} meetings")
    if results:
...
try:
    if meetings:
        ...
        if details:
            print(f"   [x] Retrieved details for: {details.get('title', 'Untitled')}")
