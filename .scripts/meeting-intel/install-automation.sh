...
if [ "$1" = "--status" ]; then
    ...
    if [ -f "$PLIST_DEST" ]; then
        echo -e "${GREEN}[x]${NC} Launch Agent installed at: $PLIST_DEST"
    else
        echo -e "${YELLOW}o${NC} Launch Agent not installed"
    fi

    if launchctl list | grep -q "$PLIST_NAME"; then
        echo -e "${GREEN}[x]${NC} Launch Agent is running"
        echo ""
        echo "Recent activity (last 10 lines):"
        tail -10 "$LOG_DIR/meeting-intel.stdout.log" 2>/dev/null || echo "  No logs yet"
    else
        echo -e "${YELLOW}o${NC} Launch Agent is not running"
    fi
...
if [ "$1" = "--stop" ] || [ "$1" = "--uninstall" ]; then
    ...
    if launchctl list | grep -q "$PLIST_NAME"; then
        ...
        echo -e "${GREEN}[x]${NC} Stopped Launch Agent"
    fi

    if [ -f "$PLIST_DEST" ]; then
        rm "$PLIST_DEST"
        echo -e "${GREEN}[x]${NC} Removed Launch Agent"
    fi
...
echo -e "${GREEN}[x]${NC} Node.js found at: $NODE_PATH"
...
if [ -f "$GRANOLA_CACHE" ]; then
    echo -e "${GREEN}[x]${NC} Granola cache found"
else
...
mkdir -p "$LOG_DIR"
echo -e "${GREEN}[x]${NC} Log directory ready"
...
echo -e "${GREEN}[x]${NC} Created: $PLIST_DEST"
...
echo -e "${GREEN}[x]${NC} Launch Agent started"

# Verify it's running
sleep 1
if launchctl list | grep -q "$PLIST_NAME"; then
    echo -e "${GREEN}[x]${NC} Verified: Launch Agent is running"
else
